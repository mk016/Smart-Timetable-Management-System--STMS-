...
export function DashboardShell({
  ...
  return (
    <div className="dashboard-grid min-h-screen bg-[#111613]">
      <aside className="border-b border-white/5 bg-forest px-4 py-5 lg:border-b-0 lg:border-r lg:px-6 lg:py-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between lg:block">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-4 sm:flex-1 lg:mt-10 lg:p-5">
        ...
        <nav className="hide-scrollbar mt-6 flex gap-2 overflow-x-auto pb-1 lg:mt-10 lg:block lg:space-y-2 lg:overflow-visible lg:pb-0">
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex shrink-0 items-center gap-3 whitespace-nowrap rounded-2xl px-4 py-3 text-sm transition",
                  active
      ...
      <main className="min-h-screen bg-canvas px-3 py-4 sm:px-4 lg:px-8 lg:py-8">{children}</main>
    </div>
  );
}
